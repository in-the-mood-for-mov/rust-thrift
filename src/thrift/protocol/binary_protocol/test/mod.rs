...
#[test]
pub fn read_bool() {
  let transport = ~FakeTransport::new(~[0x00, 0x01, 0xff]);
  let protocol = &mut BinaryProtocol::new(transport);
  assert_eq!(protocol.read_bool(), false);
  assert_eq!(protocol.read_bool(), true);
  assert_eq!(protocol.read_bool(), true);
}
...
pub fn read_binary() {
  ...
  let protocol = &mut BinaryProtocol::new(transport);
  assert_eq!(protocol.read_binary(), ~[]);
  assert_eq!(protocol.read_binary(), ~[0x41, 0x73, 0x64, 0x66]);
  assert_eq!(protocol.read_binary(), ~[0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x2c, 0x20, 0x57, 0x6f, 0x72, 0x6c, 0x64, 0x21]);
}

#[test]
pub fn read_set_begin() {
  let transport = ~FakeTransport::new(~[0x0b, 0x00, 0x00, 0x01, 0x0f]);
  let protocol = &mut BinaryProtocol::new(transport);
...
pub fn read_map_begin() {
  let transport = ~FakeTransport::new(~[0x0b, 0x08, 0x00, 0x00, 0x01, 0x0f]);
  let protocol = &mut BinaryProtocol::new(transport);
...
pub fn read_field_begin() {
  let transport = ~FakeTransport::new(~[0x00, 0x0d, 0x14, 0x0e]);
  let protocol = &mut BinaryProtocol::new(transport);
  assert_eq!(protocol.read_field_begin(), (~"", protocol::TStop, 0));
  assert_eq!(protocol.read_field_begin(), (~"", protocol::TMap, 0x140e));
}
...
pub fn read_message_begin() {
  ...
  let protocol = &mut BinaryProtocol::new(transport);
  assert_eq!(protocol.read_message_begin(), (~"foo", protocol::MtCall, 0x0002471e));
}
